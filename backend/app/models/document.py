...
class DocumentStatus(str, enum.Enum):
    UPLOADED = "uploaded"        # OCR ran successfully, awaiting staff verification
    OCR_FAILED = "ocr_failed"    # OCR could not extract text; file still stored
    VERIFIED = "verified"        # staff-approved, terminal — no further edits
    REJECTED = "rejected"        # staff-rejected — applicant may delete + re-upload


class Document(Base):
    __tablename__ = "documents"
    __table_args__ = (
        UniqueConstraint("loan_application_id", "document_type", name="uq_document_loan_type"),
    )
    ...
    # File metadata. `original_filename` is display-only and is NEVER used
    # to build a filesystem path — `stored_filename` is always a
    # server-generated UUID-based name (see file_storage.py).
    ...
    ocr_extracted_fields: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    ocr_confidence: Mapped[float | None] = mapped_column(Float, nullable=True)
    ocr_error_message: Mapped[str | None] = mapped_column(String(500), nullable=True)
